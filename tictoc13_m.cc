...
TicTocMsg13::TicTocMsg13(const char *name, int kind) : ::omnetpp::cMessage(name,kind)
{
    this->source = 0;
    this->destination = 0;
    this->receiver = 0;
...
TicTocMsg13Descriptor::TicTocMsg13Descriptor() : omnetpp::cClassDescriptor("TicTocMsg13", "omnetpp::cMessage")
{
...
bool TicTocMsg13Descriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    omnetpp::cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount())
